...
public class SWEA1247_4 {
	...
	static void dfs (int c, int d, int sum) {
		...
		// 모든 고객 대상으로 선택 (이미 선택한 고객은 skip)
		for (int i=0; i<N; i++) {
			if (visit[i]) continue;
			int dist=distance (cust[c][0], cust[c][1], cust[i][0], cust[i][1]);
			
			if (sum+dist> min) continue;	//가지치기
			visit[i]=true;
			dfs (i, d+1, sum+dist);
			visit[i]=false;
		}

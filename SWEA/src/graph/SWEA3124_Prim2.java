...
// 현재 고려 대상인 정정(간선) 중 비용이 가장 작은 것을 리턴
	...
	static void prim() {
		...
		pqueue.addAll(vertex.get(1)); // 시작정점 (1번) 에서 갈 수 있는 모든 정점(간선)을 pq 에 담는다.

		while (!pqueue.isEmpty()) {
			Edge edge = pqueue.poll();
			if (visit[edge.v])
				continue; // 새로 최소비용으로 연결하려는 정점이 이미 방문(연결)한 정점이면 skip
